...
Removing apps on Windows can be more awkward than it first appears.

- App removal entries are spread across different Windows views and registry locations.
- It is hard to select and process multiple apps in one flow.
- Important metadata such as publisher, install location, size, install date, and uninstall command is not easy to compare in one place.
- Some uninstallers fail unless they are launched with administrator permission.
- Some entries are marked with `NoRemove`, but it is not always obvious whether that means “impossible to remove” or “restricted and risky.”
- After launching an uninstaller, you often still need to manually check whether the app disappeared from the Windows app list.

This project was built to reduce those frictions. It does not blindly delete apps. Instead, it acts as a controller for discovering registered uninstall commands, reviewing them carefully, launching them, and checking the result afterward.
